...
const Dashboard = ({ lights, schedules }) => {
  ...
  useEffect(() => {
    ...
    newSocket.on('connect_error', (error) => {
      console.error('❌ Socket.IO connection error:', { SOCKET_URL, error: error.message, errorType: error.type });
    });
    ...
    // Fetch initial status with delay to ensure page is loaded
  ...
  const fetchSystemStatus = async (retryCount = 0) => {
    ...
    console.log(`🔍 Fetching system status from: ${url} (attempt ${retryCount + 1}/${maxRetries + 1})`);
    ...
    try {
      ...
      if (!response.ok) {
        console.error('❌ System status failed:', { url, status: response.status, statusText: response.statusText });
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
      
      const data = await response.json();
      console.log('✅ System status success:', { url, hasData: !!data, hasLights: !!data.lights });
      setSystemStatus(data);
      setConnectionError(null); // Clear error on success
      return true; // Success
    } catch (error) {
      console.error('❌ System status error:', { url, error: error.message, errorName: error.name, retryCount });
      
      // Retry logic
      if (retryCount < maxRetries) {
        console.log(`⏳ Retrying in ${retryDelay}ms... (${retryCount + 1}/${maxRetries})`);
        setTimeout(() => {
          fetchSystemStatus(retryCount + 1);
        }, retryDelay);
        return false; // Still retrying
      }
      
      // All retries failed
      setConnectionError(`Cannot connect to backend: ${error.message}. Please check if the backend is running at ${API_URL}`);
      return false; // Failed
    }
  };

  const fetchAiStatus = async () => {
    const url = `${API_URL}/api/ai/status`;
    try {
      const response = await fetch(url, {
        method: 'GET',
        headers: {
      ...
      // Don't show error for AI status - it's not critical
    }
  };
  ...
  const controlLight = async (room, action, brightness = 100) => {
    try {
      const response = await fetch(`${API_URL}/api/lights/${room}/control`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        fetchSystemStatus(); // Refresh status after control
        setConnectionError(null); // Clear error on success
      } else {
        const errorData = await response.json().catch(() => ({}));
        toast.error(errorData.error || `Failed to ${action} ${room} lights`);
      }
    } catch (error) {
      console.error('Error controlling lights:', error);
      toast.error(`Cannot connect to backend: ${error.message}`);
      setConnectionError(`Cannot connect to backend: ${error.message}. Backend URL: ${API_URL}`);
    }
  };
    ...
    <div className="p-6 min-h-screen bg-gray-100 dark:bg-[#0a0a0f]">
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-red-200 text-xs mt-2">
                  Backend URL: {API_URL} | Check Vercel environment variables if this is incorrect.
                </p>
              </div>
            </div>
            <button
              onClick={async () => {
                ...
                const success = await fetchSystemStatus();
                if (success) {
                  fetchAiStatus();
                }
              }}
              className="ml-4 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm transition-colors"
            >
              Retry Connection
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <motion.button
            ...
            className="btn-luxury-primary p-4 flex items-center justify-center"
          >
        ...
        <motion.button
          ...
          className="btn-luxury-secondary text-white dark:text-white p-4 flex items-center justify-center"
        >
